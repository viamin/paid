...
module AgentHarness
  module Orchestration
    ...
    class ProviderManager
      ...
      def select_provider(preferred = nil, allowed_providers: nil)
        preferred ||= @current_provider

        unless provider_allowed?(preferred, allowed_providers)
          return select_fallback(preferred, reason: :not_allowed, allowed_providers: allowed_providers)
        end

        # Check circuit breaker
        if circuit_open?(preferred)
          return select_fallback(preferred, reason: :circuit_open, allowed_providers: allowed_providers)
        end

        # Check rate limit
        if rate_limited?(preferred)
          return select_fallback(preferred, reason: :rate_limited, allowed_providers: allowed_providers)
        end

        # Check health
        unless healthy?(preferred)
          return select_fallback(preferred, reason: :unhealthy, allowed_providers: allowed_providers)
        end
      ...
      def switch_provider(reason:, context: {}, from: nil, allowed_providers: nil)
        old_provider = from || @current_provider

        fallback = select_fallback(old_provider, reason: reason, allowed_providers: allowed_providers)
      ...
      # Providers configured to serve embeddings for the requested model.
      #
      # @param model [String] embedding model identifier
      # @return [Array<Symbol>] provider names
      def providers_supporting_embeddings(model:)
        @config.providers.each_with_object([]) do |(name, provider_config), providers|
      ...
      def select_fallback(provider_name, reason:, allowed_providers: nil)
        ...
        chain.each do |fallback_name|
          ...
          next unless provider_allowed?(fallback_name, allowed_providers)
        ...
        raise NoProvidersAvailableError.new(
          ...
          attempted_providers: filtered_chain(chain, allowed_providers),
      ...
      def provider_allowed?(provider_name, allowed_providers)
        return true unless allowed_providers

        allowed_providers.include?(provider_name.to_sym)
      end

      def filtered_chain(chain, allowed_providers)
        return chain unless allowed_providers

        chain.select { |name| provider_allowed?(name, allowed_providers) }
      end

      def provider_supports_model?(provider_config, model)
        return true if model.nil?
        return provider_config.models.include?(model) if provider_config.models.any?
        return provider_config.model == model if provider_config.model
