...
module AgentHarness
  module Orchestration
    ...
    class Conductor
      ...
      #
      # Restricts fallback candidates to providers configured for the same
      # model so failover stays within a compatible embedding space.
      #
      # @param texts [Array<String>] input texts
      # @param provider [Symbol, nil] preferred provider
      # @param model [String] embedding model identifier
      # @param dimensions [Integer, nil] optional dimensions override
      # @param options [Hash] additional provider options
      # @return [EmbeddingResponse]
      def embed(texts:, provider: nil, model:, dimensions: nil, **options)
        provider_name = provider || @config.default_provider
        allowed_providers = @provider_manager.providers_supporting_embeddings(model: model)

        if allowed_providers.empty?
          raise NoProvidersAvailableError.new(
            "No embedding providers configured for model #{model}",
            attempted_providers: [],
            errors: {}
          )
        end

        with_orchestration(provider_name, model, options, allowed_providers: allowed_providers) do |selected_provider|
          selected_provider.embed(texts: texts, model: model, dimensions: dimensions, **options)
        end
      end
      ...
      def with_orchestration(provider_name, model, options, allowed_providers: nil)
        retries = 0
        ...
        begin
          ...
          provider = @provider_manager.select_provider(provider_name, allowed_providers: allowed_providers)
          ...
          handle_provider_failure(e, provider_name, :switch, allowed_providers: allowed_providers)
          ...
          handle_provider_failure(e, provider_name, :retry, allowed_providers: allowed_providers)
          retry if should_retry?(retries += 1, max_retries)
          raise
      ...
      def handle_provider_failure(error, provider_name, strategy, allowed_providers: nil)
