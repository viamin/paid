...
- **11 Built-in Providers**: CLI agents plus OpenAI-compatible embedding endpoints
...
# Generate embeddings through an OpenAI-compatible provider
embedding = AgentHarness.embed(
  texts: ["chunk 1", "chunk 2"],
  provider: :openai,
  model: "text-embedding-3-large",
  dimensions: 3072
)
puts embedding.vectors.length
puts embedding.token_count
```
...
| `:openai` | `http` | OpenAI-compatible embeddings API |
| `:openrouter` | `http` | OpenRouter embeddings API |
| `:azure_openai` | `http` | Azure OpenAI embeddings API |
...
`AgentHarness.embed` uses the same orchestration layer as `send_message`, including
fallback, circuit breakers, and rate-limit handling. Fallback candidates are
restricted to providers configured for the same embedding model so failover does
not switch across incompatible vector spaces.

```ruby
AgentHarness.configure do |config|
  config.default_provider = :openai
  config.fallback_providers = [:azure_openai, :openrouter]

  config.provider(:openai) do |p|
    p.model = "text-embedding-3-large"
    p.api_key_env_var = "OPENAI_API_KEY"
  end

  config.provider(:azure_openai) do |p|
    p.model = "text-embedding-3-large"
    p.base_url = "https://my-resource.openai.azure.com"
    p.api_key_env_var = "AZURE_OPENAI_API_KEY"
    p.deployment = "text-embedding-3-large"
    p.api_version = "2024-10-21"
  end

  config.provider(:openrouter) do |p|
    p.model = "text-embedding-3-large"
    p.api_key_env_var = "OPENROUTER_API_KEY"
  end
end
```
