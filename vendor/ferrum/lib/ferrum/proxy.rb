...
module Ferrum
  class Proxy
    class << self
      ...
      def proxy_server_class
        return const_get(:HTTPProxyServer, false) if const_defined?(:HTTPProxyServer, false)

        load_webrick!

        const_set(:HTTPProxyServer, Class.new(WEBrick::HTTPProxyServer) do
          # rubocop:disable all
      ...
      def load_webrick!
        ...
        raise LoadError, "#{e.message} — Ferrum proxy support requires the optional `webrick` gem."
      end
    end
    ...
    def start
      ...
      @server = self.class.proxy_server_class.new(**options)
      @server.start
      at_exit { stop }
