...
# Extract install contract metadata from agent-harness for a given runner.
#
...
# install recipe into the Dockerfile without hardcoding runner-specific
# install logic in Paid.
#
# Usage:
#   bundle exec ruby scripts/extract-runner-install-contract.rb claude
...
runner = ARGV[0]
unless runner
  warn "Usage: #{$PROGRAM_NAME} <runner>"
  exit 1
end
...
if source == :uv_tool
  ...
  puts "BUN_INSTALL_SCRIPT_URL=#{bun_requirement[:install_script_url]}" if bun_requirement
else
