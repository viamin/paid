#!/bin/bash
# Runner-contract smoke test for the paid-agent image.
# Validates that Paid's runnable-runner contract and generated config shape
# are compatible with the CLIs installed in the agent image.
#
# Usage:
#   ./scripts/test-agent-runner-contracts.sh
#   IMAGE_NAME=myregistry/paid-agent ./scripts/test-agent-runner-contracts.sh

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
INNER_SCRIPT="${SCRIPT_DIR}/test-agent-runner-contracts-inner.sh"
...
CONTAINER_EXECUTABLE_KEYS=$(cd "${PROJECT_ROOT}" && "${RUBY_CONTRACT_ENV[@]}" bundle exec ruby -e "
  ...
  require 'runner_support'
  puts RunnerSupport.container_executable_runner_keys.sort.join(' ')
")
...
CODEX_CONFIG_TOML_BODY=$(cd "${PROJECT_ROOT}" && "${RUBY_CONTRACT_ENV[@]}" bundle exec ruby -e "
  ...
  puts runner.config_file_content(
    model_provider: 'test',
...
echo ""
echo "============================================="
echo "Runner-contract smoke test completed successfully!"
