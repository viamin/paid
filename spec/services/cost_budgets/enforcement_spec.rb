...
RSpec.describe "Cost budget enforcement", type: :model do
  let(:project) { create(:project) }

  describe "ProcessRunQueueJob budget check" do
    let(:agent_run) { create(:agent_run, :running, project: project) }

    it "fails run when hard_stop budget is exceeded" do
      ...
      result = CostBudgets::Check.call(project)

      expect(result[:allowed]).to be false
      expect(result[:reason]).to include("daily budget exceeded")
    end

    it "allows run when only alert-mode budgets are exceeded" do
