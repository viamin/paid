...
RSpec.describe PromptAssembly::Build, :no_db do
  ...
  describe "result provenance" do
    ...
    it "exposes prompt_digest, profile fingerprint, and budget decisions in provenance" do
      profile = PromptAssembly::Profile.new(budgets: { knowledge: { tokens: 1000 } })
      result = described_class.call(
        profile: profile,
        sections: [ section(key: :knowledge, content: "Knowledge") ]
      )

      expect(result.provenance[:prompt_digest]).to eq(result.prompt_digest)
      expect(result.provenance[:profile_fingerprint]).to eq(result.profile_fingerprint)
      expect(result.provenance[:budget_decisions]).to eq(result.budget_decisions)
    end
  end
end
