...
RSpec.describe PromptAssembly::GoalAssembly do
  ...
  describe "provenance and digest" do
    ...
    it "includes the prompt digest, profile fingerprint, and budget decisions" do
      agent_run = create(:agent_run, :create_issue_goal, project: project)
      goal_text = "#{base_prompt}\n\nIMPORTANT: create a GitHub issue"
      ...
      provenance = result.provenance
      expect(provenance[:prompt_digest]).to eq(result.prompt_digest)
      expect(provenance[:profile_fingerprint]).to eq(result.profile_fingerprint)
      expect(provenance[:budget_decisions]).to eq(result.budget_decisions)
    end
