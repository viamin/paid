...
RSpec.describe AppleVerification::ExecuteGuestJob do # @spec APPLE-VERIFY-005
  ...
  it "selects the account's active image and dispatches the validated manifest" do
    image = create(:apple_verification_image, :active, account: project.account)

    result = described_class.call(project:, manifest:, adapters:)
  ...
  it "does not dispatch when the rollout is disabled" do
    ...
    expect { described_class.call(project:, manifest:, adapters:) }
      .to raise_error(described_class::FeatureDisabledError)
  end

  it "does not dispatch when the account has no active image" do
    create(:apple_verification_image, account: project.account)
  ...
  it "does not dispatch an invalid manifest" do
    create(:apple_verification_image, :active, account: project.account)

    expect { described_class.call(project:, manifest: manifest.merge("version" => 2), adapters:) }
