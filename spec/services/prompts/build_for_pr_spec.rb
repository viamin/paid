...
RSpec.describe Prompts::BuildForPr do
  ...
  describe "conversation comments section" do
    ...
    it "requests a bounded recent comment window" do
      described_class.call(
        project: project,
        pr_number: 42,
        github_client: github_client,
  ...
  describe "conversation comment limits" do
    ...
    it "keeps only the last configured number of trusted comments" do
      comments = (1..5).map do |i|
      ...
      expect(prompt).to include("Comment 5")
      expect(prompt).to include("Comment 3")
      expect(prompt).not_to include("Comment 2")
    end

    it "truncates each comment body using the issue prompt limit" do
      allow(github_client).to receive(:recent_issue_comments)
        .with(project.full_name, 42, pages: described_class::RECENT_COMMENT_PAGE_WINDOW)
        .and_return([
          OpenStruct.new(user: OpenStruct.new(login: "trusteduser"), body: "A" * 150)
        ])
      ...
      expect(prompt).not_to include("A" * 150)
    end
  end
