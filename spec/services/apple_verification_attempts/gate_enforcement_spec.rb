...
RSpec.describe AppleVerificationAttempts::GateEnforcement do
  context "when execution is unavailable" do
    ...
    it "keeps completion verification out of the released gate path" do
      ...
      expect(decision).not_to be_enforcing
      expect(decision.gate).to eq("completion_verification")
      expect(decision.attempt).to be_nil
      expect(decision.revision).to be_nil
      expect(decision.reason).to include("not released")
      expect(attempt).to be_failed
    end

    it "keeps pull-request verification out of the released gate path" do
