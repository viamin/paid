...
RSpec.describe ExecutionRunners::LocalDockerRunner do
  ...
  describe ".compatible?" do
    ...
    # RDR-055: a backend that shares no filesystem rejects the legacy worktree
    # path, and the runner must forward that host path so the rejection reaches
    # scheduling instead of failing at provision time.
    it "surfaces the backend's rejection of a workspace that needs shared host storage" do
      bind_mount_spec = ExecutionRunners::RunSpec.new(**run_spec.to_h.merge(
        workspace: ExecutionRunners::WorkspaceStrategy.bind_mount(reference: "/var/paid/worktrees/1")
      ))
      allow(Containers::Provision).to receive(:compatibility_for)
        .with(agent_run: agent_run, backend: backend, worktree_path: "/var/paid/worktrees/1")
        .and_return(Containers::Provision::CompatibilityResult.new(
          compatible: false, error_message: "worktree path /var/paid/worktrees/1 requires a host bind mount"
        ))

      result = described_class.compatible?(spec: bind_mount_spec, backend: backend)

      expect(result.compatible).to be(false)
      expect(result.error_message).to eq("worktree path /var/paid/worktrees/1 requires a host bind mount")
    end
  end
  ...
  # RDR-055 conformance: the runner must satisfy the contract on a backend that
  # does NOT expose host paths, proving normal create-PR execution never depends
  # on shared host storage. +backend+ is overridden (rather than introducing a
  # second double) so the backend the runner resolves through
  # +Containers.backend_for+ is the same one the conformance examples assert on.
  it_behaves_like "a no-shared-filesystem runner" do
    let(:agent_run) { create(:agent_run, container_host: "remote") }
    let(:backend) do
      instance_double(Containers::Backends::Base, identifier: "remote", remote?: true, supports_host_paths?: false)
    end
    let(:valid_handle) do
      ExecutionRunners::RunnerHandle.new(runner_type: :local_docker, identifier: "abc123", host: "remote",
        workspace_ref: described_class.workspace_volume_name_for(agent_run.id),
        metadata: { "agent_run_id" => agent_run.id, "worktree_path" => nil, "environment" => {} })
    end

    before do
      allow(Containers::Provision).to receive_messages(new: provision_service, reconnect: provision_service)
      allow(backend).to receive(:stop_container)
      allow(provision_service).to receive_messages(
        provision: Containers::Provision::Result.success(container_id: "abc123", container_host: "remote"),
        execute: Containers::Provision::Result.success(stdout: "ok\n", stderr: "", exit_code: 0),
        container_running?: true, backend: backend, cleanup: nil
      )
    end
  end
end
