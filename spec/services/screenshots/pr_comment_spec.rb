...
RSpec.describe Screenshots::PrComment do
  ...
  describe "trace viewer link" do
    let(:trace_viewer_url) { "https://example.test/trace-viewer/index.html&trace=..." }

    def body_with_trace(url:)
      described_class.new(
        github_client: github_client,
        repo: repo,
        pr_number: pr_number,
        commit_sha: commit_sha,
        screenshots: screenshots,
        trace_viewer_url: url
      ).build_comment_body
    end

    it "appends an interactive trace section linking to the viewer" do
      body = body_with_trace(url: trace_viewer_url)

      expect(body).to include("### Interactive Trace")
      expect(body).to include("[Open Playwright trace viewer](#{trace_viewer_url})")
    end

    it "places the trace section after the screenshots and before the footer" do
      body = body_with_trace(url: trace_viewer_url)

      expect(body.index("### Interactive Trace")).to be > body.index(screenshots.first[:route_name])
      expect(body.index("### Interactive Trace")).to be < body.index("_Generated by Paid_")
    end

    it "omits the trace section when no viewer URL is provided" do
      body = described_class.new(
        github_client: github_client,
        repo: repo,
        pr_number: pr_number,
      ...
      expect(body).not_to include("Interactive Trace")
    end
  end
end
