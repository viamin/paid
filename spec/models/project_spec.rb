...
RSpec.describe Project do
  ...
  describe "validations" do
    ...
    # @spec PR-ESCALATION-024
    it "rejects an approval-wait escalation ceiling beyond 30 days" do
      project = build(:project)
      project.pr_approval_escalation_hours = 721
  ...
  describe "review_settings" do
    ...
    describe "#effective_feature_activation_labels" do
      ...
      it "defaults the auto-pick activation label to the project's automation label name" do
        project = create(:project, feature_activation_labels: nil, automation_label_name: "run-paid")

        expect(project.feature_activation_label_for("auto_pick")).to eq("run-paid")
      end

      it "prefers an explicit auto-pick activation label over the automation label name" do
        project = create(:project, feature_activation_labels: { "auto_pick" => "queue-paid" })

        expect(project.feature_activation_label_for("auto_pick")).to eq("queue-paid")
      end
