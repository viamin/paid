...
RSpec.describe RunnerSupport do
  before do
    described_class.reset_supported_runner_keys!
  end
  ...
  describe "CONTAINER_EXECUTABLE_RUNNER_KEYS" do
    it "includes claude" do
      expect(described_class::CONTAINER_EXECUTABLE_RUNNER_KEYS).to include("claude")
    end

    it "includes codex" do
      expect(described_class::CONTAINER_EXECUTABLE_RUNNER_KEYS).to include("codex")
    end

    it "includes kilocode" do
      expect(described_class::CONTAINER_EXECUTABLE_RUNNER_KEYS).to include("kilocode")
    end

    it "includes cursor" do
      expect(described_class::CONTAINER_EXECUTABLE_RUNNER_KEYS).to include("cursor")
    end
  ...
  describe ".container_executable_runner_key?" do
    it "returns true for codex" do
      expect(described_class.container_executable_runner_key?("codex")).to be true
    end

    it "returns true for claude" do
      expect(described_class.container_executable_runner_key?("claude")).to be true
    end

    it "returns true for cursor" do
      expect(described_class.container_executable_runner_key?("cursor")).to be true
    end

    it "returns true for kilocode" do
      expect(described_class.container_executable_runner_key?("kilocode")).to be true
    end
    ...
    it "returns false for unsupported providers" do
      expect(described_class.container_executable_runner_key?("unknown_provider")).to be false
    end
    ...
    it "returns false for supported providers not in the container-executable set" do
      ...
      expect(described_class.container_executable_runner_key?("codex")).to be false
    end
  end
  ...
  describe ".supported_runner_keys" do
    it "excludes providers not registered in the harness" do
      ...
      supported_keys = described_class.supported_runner_keys
  ...
  describe "provider contract smoke checks" do
    describe "CLI binary mapping completeness" do
      it "has a CLI binary mapping for every container-executable runner" do
        cli_binary_for = {
          "aider" => "aider",
          "claude" => "claude",
          "codex" => "codex",
    ...
    describe "agent-harness registry backing" do
      it "only contains runners backed by the agent-harness registry" do
        described_class::CONTAINER_EXECUTABLE_RUNNER_KEYS.each do |key|
