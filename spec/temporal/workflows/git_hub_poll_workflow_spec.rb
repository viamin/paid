...
RSpec.describe Workflows::GitHubPollWorkflow do
  ...
  describe "#handle_pr_scan_results" do
    ...
    def run_pr_scan_results(sidecar_patch_enabled:, workflow_instance: workflow)
      allow(workflow_instance).to receive(:run_activity).and_return({})
      allow(Temporalio::Workflow).to receive(:patched)
        .with("queue-paid-agent-review-sidecar-v1")
        .and_return(sidecar_patch_enabled)
      allow(Temporalio::Workflow).to receive(:patched)
        .with("draft-followup-direct-start-v1")
        .and_return(true)
      allow(Temporalio::Workflow).to receive(:patched)
      ...
      workflow_instance.send(:handle_pr_scan_results, scan_result, project_id)
    end

    before do
      run_pr_scan_results(sidecar_patch_enabled: true)
    end
    ...
    it "skips the paid_agent review sidecar before the patch while still handling the trigger" do
      workflow = described_class.new
      run_pr_scan_results(sidecar_patch_enabled: false, workflow_instance: workflow)
  ...
  describe "initial sync for existing PRs" do
    ...
    before do
      ...
      allow(Temporalio::Workflow).to receive(:patched)
        .with("queue-paid-agent-review-sidecar-v1").and_return(true)
