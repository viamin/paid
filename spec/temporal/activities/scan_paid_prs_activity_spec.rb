...
RSpec.describe Activities::ScanPaidPrsActivity do
  ...
  describe "#execute" do
    ...
    context "when both copilot and codex are enabled but only copilot has reviewed" do
      before do
        ...
        stub_github_for_pr(
          checks: [ { name: "ci", conclusion: "success", status: "completed" } ],
          reviews: [ { id: 1, user_login: "copilot", state: "COMMENTED",
                       body: "Everything looks good!", submitted_at: 1.hour.ago } ],
          review_threads: []
        )
      end

      it "still requires a codex review even though copilot review is clean" do
        result = activity.execute(project_id: project.id)

        expect(result[:prs_to_trigger].size).to eq(1)
        trigger_types = result[:prs_to_trigger].first[:triggers].map { |t| t[:type] }
        expect(trigger_types).to include("review_bot_review_pending")
        pending_trigger = result[:prs_to_trigger].first[:triggers].find { |t| t[:type] == "review_bot_review_pending" }
        expect(pending_trigger[:request_login]).to eq("chatgpt-codex-connector")
      end
    end
