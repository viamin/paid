  ...
  describe "#execute" do
    ...
    context "when paid_agent review does not include a clean signal" do
      ...
      it "proceeds through normal flow instead of bypassing via clean signal" do
        ...
        trigger_types = result[:prs_to_trigger].flat_map { |t| t[:triggers].map { |x| x[:type] } }
        # With paid_agent registered as a bot, the scanner sees no bot
        # review yet and emits review_bot_review_pending to request one —
        # proving the non-clean human review did not trigger the clean
        # signal bypass.
        expect(trigger_types).to include("review_bot_review_pending")
    ...
    context "when a newer non-clean review follows an older clean paid_agent review" do
      ...
      it "does not treat the review as clean because the latest review is non-clean" do
        ...
        trigger_types = result[:prs_to_trigger].flat_map { |t| t[:triggers].map { |x| x[:type] } }
        # The latest review lacks the clean marker, so the bypass is
        # skipped. With paid_agent registered as a bot, the scanner sees
        # no bot review and emits review_bot_review_pending to request one.
        expect(trigger_types).to include("review_bot_review_pending")
        expect(trigger_types).not_to include("ready_for_owner")
      end
    end
