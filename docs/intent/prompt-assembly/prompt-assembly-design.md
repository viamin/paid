...
## Migration Status (final audit, #3381)

### Fully migrated

- **PR follow-up path** (`#3378`): `Prompts::BuildForPr` assembles all sections
  through `PromptAssembly::Build` with full provenance. Each section is a
  `PromptAssembly::Section` with key, source, trust level, and required flag.
  Untrusted review-thread and conversation comments are classified via
  `PromptAssembly::Trust` and recorded as excluded provenance. Profile
  resolution applies project/account/global overrides. `PreparePrPromptActivity`
  persists the assembly result (sections, skipped, digest, fingerprint, budget
  decisions) on the `prepare_pr_prompt` phase metadata.
- **Goal wrappers** (`#3379`): `RunAgentActivity#augment_prompt_for_goal`
  routes every agent run through `PromptAssembly::GoalAssembly`, which
  contributes the create-issue, review, enhance-issue, and interactive
  verification wrappers as `required` Sections. Provenance is recorded on the
  run via `AgentRun#record_prompt_assembly!`.
- **Trusted-input filtering** (`#3376`): `PromptAssembly::Trust` and
  `TrustedInput` centralize the allowlist-based classification used by both
  `BuildForIssue#fetch_trusted_comments` and `BuildForPr`.
- **Assembly profiles** (`#3380`): `PromptAssembly::Profile` and
  `ProfileResolution` resolve ordering, optional section disabling, and budgets.
  Safety-critical (`required`) sections are structurally immune to suppression.

### Documented follow-up: BuildForIssue internal sections

`Prompts::BuildForIssue` (`#3377`) uses `PromptAssembly::Trust` for
comment-filtering and `Knowledge::ContextBundle::Build` for quarantined context,
but its own section composition (task template, conversation comments,
clarifying answers, service environment) still uses string concatenation rather
than `PromptAssembly::Build`. The trust boundary IS enforced: comments are
allowlist-filtered, knowledge is quarantined, and the base prompt is wrapped by
`GoalAssembly` at runner time. Migrating `BuildForIssue`'s internal sections to
explicit `PromptAssembly::Section`s is filed as a follow-up so the prompt text
is not changed in the audit pass. See RDR-054 "Outstanding Questions" and the
follow-up issue linked there.

### Still call-site injectors (not yet registered providers)
...
- `Lid::InjectIntoPrompt`, `StyleGuides::InjectIntoPrompt`, and
  ...
  inside the existing builders rather than registered section providers.
- `CreateAgentRunActivity` queue-time materialization renders the
  `coding.issue_implementation` prompt version into `custom_prompt`. The base
  text it produces is captured by the assembly's `task.base` section and digest
  at runner time, but the queue-time inputs themselves are not yet expressed as
  assembly sections.
