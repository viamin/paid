...
the project's effective gate settings and any enabled `QualityThreshold`
records scoped to that project (see "Unified Quality Thresholds" below).
...
`QualityThreshold` is a single account/project-scoped model that backs two
related but distinct features (issue #3514 merged the formerly separate
`QualityGateThreshold` model into it):

- **Goal-scoped defaults** — records keyed by `(metric_type, goal_type)` that
  drive `QualityThreshold.effective_for`/`.configurable_for`, consumed by
  `QualityPause::Check` (auto-pause/recovery), the quality dashboard's
  threshold-editing table, prompt-evolution sampling, and trend analysis.
- **Project-scoped gate thresholds** — records keyed by `(metric_type,
  goal_type: "all")`, using the reserved `QualityThreshold::ALL_GOALS`
  sentinel to mean "regardless of the triggering run's goal". These support
  an optional `max_value` ceiling and a `severity` (`info`/`warning`/
  `critical`), and are consumed only by `QualityMetrics::EvaluateGate`
  (nightly mutation-sweep breach/recovery events, recorded as
  `QualityGateEvent`) and `Activities::CheckQualityGateActivity#threshold_breaches`
  (the per-run admission gate).

The two families never overlap: `QualityThreshold.account_thresholds`/
`.project_thresholds` (the private query helpers behind `effective_for` and
`configurable_for`) explicitly exclude `goal_type: "all"` rows, so gate
thresholds cannot leak into the goal-scoped default/override system, and
`goal_type: "all"` is never generated by the built-in default/supported
definitions. Gate thresholds have no dedicated controller or route today —
