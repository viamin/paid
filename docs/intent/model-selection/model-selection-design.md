...
catalog defaults. GPT-5.6 Luna, Terra, and Sol have subscription support and
retain their low/mid/high tier labels. A verified runner-local replacement from
RUNNER-FALLBACK-008 takes precedence over stale catalog or static compatibility
information, while explicit project restrictions and operator disables still
apply. Catalog refreshes do not erase execution evidence. Model recovery may
supply an explicit policy-filtered candidate pool to the meta-agent and allow
another tier when no suitable same-tier candidate remains.
