...
| Approved image identity | The local Tart VM directory under `<TART_HOME>/vms/<name>/` hashes (recursive SHA-256 over every non-hidden file in sorted order) to a value matching one of the `AppleVerificationImage#digest` entries bound to the account | Either re-clone the immutable image via `tart clone <source>` or publish a fresh `AppleVerificationImage` row through the admin UI; the report prints the published digests and the local VM names so the operator can reconcile. `tart list` does not emit image digests; the check is anchored on the directory contents Tart writes under `<TART_HOME>/vms/`. |
