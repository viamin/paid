...
- [x] **AUTO-MERGE-008** — When project-level auto-merge is off, a trusted
  ...
  that pull request only — through the Dependabot auto-merge job for
  Dependabot PRs and through the scanner's auto-merge evaluation plus
  `MergePullRequestActivity` for human/Paid PRs. `paid-in-full` alone SHALL NOT
  grant merge authority; it must be paired with the PR activation label or the
  normal project-level auto-merge setting.
  *Code:* `app/services/automation/feature_activation.rb`,
  `app/services/automation/strategies/auto_merge.rb`,
  `app/temporal/activities/scan_paid_prs_activity.rb`,
  `app/temporal/activities/merge_pull_request_activity.rb`,
  `app/temporal/activities/evaluate_dependabot_auto_merge_activity.rb`,
