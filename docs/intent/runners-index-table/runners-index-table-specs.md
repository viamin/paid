...
- [x] **RUNNERS-INDEX-007** — When the `/runners` index no longer renders an
  on-page auth setup guide, the "Test Runner" authentication troubleshooting
...
- [x] **RUNNERS-INDEX-009** — When the `/runners` index renders at least one
  configured runner, it SHALL render a header-level `Test All` control above
  the table, and when no configured runners are present it SHALL omit that
  control.
  *Code:* `app/views/runners/index.html.erb`.
  *Test:* `spec/requests/runners_spec.rb`.

- [x] **RUNNERS-INDEX-010** — When an operator activates the `/runners` index
  `Test All` control, the page SHALL invoke the existing per-row `test-agent`
  controller for each rendered runner row, keep the control disabled while the
  batch is running, update the control label with completion progress, and
  allow one row's cooldown or failure result to settle without aborting the
  other row tests.
  *Code:* `app/javascript/controllers/test_all_controller.js`.
  *Test:* `spec/lib/test_all_controller_node_harness_spec.rb`.
