...
related to a closed issue. Reopening is an exception: a caller authorized to
manage issues must provide the normal write confirmation, an explicit
...
issue and in the account audit trail. While the synchronized reopen review is
pending, the tool refuses a close transition. These checks make an agent's
confirmation intentional and prevent it from immediately undoing the gate it
triggered.
