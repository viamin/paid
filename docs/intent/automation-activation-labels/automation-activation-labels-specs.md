...
- [x] **AUTOMATION-ACTIVATION-002** — `Projects::EnsureStandardLabels` SHALL
  ...
  reconciling drift by configured name rather than hard-coded literal. When
  the `auto_pick` activation label resolves to `automation_label_name`, the
  label SHALL be provisioned exactly once (no cross-category collision).
...
- [x] **AUTOMATION-ACTIVATION-003** — When a project-level automation setting
  ...
  convert the project into an allowlist. The issue-side activation path SHALL
  still require `automation_on_label_enabled`; with the label master switch
  off, activation labels SHALL NOT queue issue work. With `auto_scan_prs` on,
  the PR scan set SHALL remain limited to automation-labeled PRs.
...
- [x] **AUTOMATION-ACTIVATION-004** — Issue-scoped activation SHALL respect
  ...
  *Code:* `app/services/automation/feature_activation.rb`,
  `app/services/automation/label_policy.rb`.
  *Test:* `spec/services/automation/feature_activation_spec.rb`,
  `spec/services/automation/issue_evaluator_spec.rb`,
  `spec/temporal/activities/detect_labels_activity_spec.rb`.
