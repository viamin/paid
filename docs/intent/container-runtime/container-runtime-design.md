...
- Value objects consolidate existing patterns: `RunSpec` (what to run),
  ...
  `ServiceDeclaration`, `ComputeRequirements`, and `AuthorityGrant`
  (secret-free per-run credential authority, RDR-056).
...
### Per-run execution authority grants (RDR-056)

`ExecutionRunners::AuthorityGrant` represents which credential classes a run
is allowed to receive or access, and how they flow (proxy, subscription, or
direct outbound). It is carried on `RunSpec` as a first-class field so grant
decisions are explicit before provisioning and a runner can validate support.

The grant is secret-free by construction: `CredentialGrant` carries only
non-secret metadata (provider hints, scope identifiers, counts). No token
value, API key, or other secret material is stored in the grant or its
serialized `to_summary` form, so the grant can be inspected, persisted, and
audited without redaction.

Three modes are distinguishable via predicates:

- `:proxy` (RDR-006 default) — provider calls flow through Paid's secrets
  proxy. The run receives `paid_proxy_token`, `github_authority`, and
  `model_provider_proxy` grants.
- `:subscription_auth` — host-backed provider CLI auth. The run receives
  `subscription_auth` alongside the proxy token and GitHub authority.
- `:direct_outbound` — provider bypasses the proxy entirely. The run receives
  `model_provider_direct` and GitHub authority.

Additional grants (MCP credentials, object-storage upload authority, service
credentials) are passed by callers that provision those resources:

- `:mcp_credentials` — MCP server credentials for configured MCP servers.
