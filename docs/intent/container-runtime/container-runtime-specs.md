...
- [x] **CONTAINER-RUNTIME-018** — The system SHALL define an
  `ExecutionRunners::AuthorityGrant` value object that represents which
  credential classes a run is allowed to receive or access (`paid_proxy_token`,
  `github_authority`, `model_provider_proxy`, `model_provider_direct`,
  `subscription_auth`, `mcp_credentials`, `object_storage_upload`,
  `service_credentials`) and carries a `mode` (`:proxy`, `:subscription_auth`,
  `:direct_outbound`) so grant decisions are explicit before provisioning and a
  runner can validate support. The grant SHALL be secret-free: no token, key,
  or credential value is stored in the grant or its serialized form, so it can
  be inspected, persisted, and audited without exposing secrets. Proxy-mode
  grants SHALL be distinguishable from subscription-auth and direct-outbound
  grants via `proxy?`, `subscription_auth?`, and `direct_outbound?` predicates.
  `RunSpec` SHALL carry the grant so the runner receives it as a first-class
  contract field alongside `NetworkingPolicy`.
  *Tests:* `spec/services/execution_runners_spec.rb`
  *Code:* `ExecutionRunners::AuthorityGrant`,
  `ExecutionRunners::CredentialGrant`, `ExecutionRunners::RunSpec`
