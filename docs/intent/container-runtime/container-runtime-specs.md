...
- [x] **CONTAINER-RUNTIME-007** — The system SHALL define a provider-neutral
  ...
  detection). The interface SHALL remain satisfiable on a backend whose
  `supports_host_paths?` is false (RDR-055): a runner SHALL provision, execute a
  workload, report its outcome, and tear down without any host filesystem path.
...
- [x] **CONTAINER-RUNTIME-008** — `ExecutionRunners::RunnerHandle` SHALL be
  ...
  `RunnerHandle.from_record` / `RunnerHandle#to_storage` for DB round-trip. The
  handle's `workspace_ref` SHALL carry the runner-owned storage reference (never
  a host bind-mount path) and the persisted handle alone SHALL be enough to
  observe, cancel, and tear down the environment on a backend whose
  `supports_host_paths?` is false (RDR-055).
...
- [x] **CONTAINER-RUNTIME-009** — The system SHALL define immutable value
  ...
  `NetworkPolicy::NetworkContract` without Docker-specific identifiers. The
  value objects SHALL carry a create-PR run (spec → provision → execute →
  result → handle recovery → cleanup) on a backend whose `supports_host_paths?`
  is false (RDR-055), with the workload's output and outcome reported on
  `ExecutionResult` rather than through a host file, proving the
  provider-neutral shapes carry the runner contract beyond Docker.
  *Tests:* `spec/services/execution_runners_spec.rb`,
  `spec/services/execution_runners/base_no_shared_filesystem_conformance_spec.rb`
...
- [x] **CONTAINER-RUNTIME-010** — The system SHALL provide a
  ...
  (Docker-only) backends. The runner SHALL also satisfy the no-shared-FS
  conformance contract (RDR-055) when pointed at a backend whose
  `supports_host_paths?` is false, and SHALL forward a legacy bind-mount
  worktree path to `Containers::Provision.compatibility_for` so that backend's
  rejection reaches scheduling instead of failing at provision time.
...
- [x] **CONTAINER-RUNTIME-011** — The system SHALL express workspace storage as
  ...
  A runner implementation SHALL be able to provision and clean up a
  `WorkspaceStrategy` on a backend whose `supports_host_paths?` is false, and
  SHALL refuse a `:bind_mount` strategy there rather than reaching for a host
  reference (RDR-055).
  *Tests:* `spec/services/execution_runners_spec.rb`,
  `spec/services/execution_runners/local_docker_runner_spec.rb`,
  `spec/services/execution_runners/base_no_shared_filesystem_conformance_spec.rb`,
