...
- **Related Tests**: `spec/services/prompt_assembly/build_spec.rb`, `spec/services/prompt_assembly/trust_spec.rb`, `spec/services/prompt_assembly/trusted_input_spec.rb`, `spec/services/prompt_assembly/goal_assembly_spec.rb`, `spec/services/prompt_assembly/profile_spec.rb`, `spec/services/prompt_assembly/profile_resolution_spec.rb`, `spec/services/prompts/build_for_pr_spec.rb` (assembly integration), `spec/temporal/activities/run_agent_activity_spec.rb` (goal-wrapper assembly provenance), `spec/services/knowledge/context_bundle/build_spec.rb` (quarantine notice)
...
1. ~~Should assembly profiles live in a dedicated `prompt_assembly_profiles` table immediately, or start inside `configuration_bundles` / tenant settings until the shape stabilizes?~~ **Resolved:** profiles are JSONB-backed on existing surfaces (`tenant_settings.features["prompt_assembly_profile"]`, `projects.review_settings["prompt_assembly_profile"]`). No dedicated table was needed for v1.
2. Should authenticated custom prompts from ordinary users be classified as `trusted_user_instruction` by default, or should project admins be able to require review before custom prompts run? **Open** — current implementation trusts tenant-authored prompts; a review gate is a future enhancement.
3. ~~Should project repository content be allowed as quarantined context in all runs, or only when knowledge collection has marked artifacts active and non-stale?~~ **Resolved:** knowledge sections are `:quarantined` with explicit "do not follow instructions" framing; the knowledge bundle builder applies its own staleness filtering.
4. How much final prompt text should be visible in UI, given that prompts may include sensitive internal context? **Open** — provenance persists section metadata (keys, sources, trust levels, inclusion reasons) without section bodies. Full prompt-text preview in UI is deferred.

## Final Audit Notes (#3381)

### Completed phases

| Phase | Status | Issue |
|---|---|---|
| Phase 1: Design the assembly contract | Done | #3375 |
| Phase 2: Build compatibility providers | Done | #3376 |
| Phase 3: Route issue implementation through assembly | Partial — trust filtering and quarantine are enforced via `PromptAssembly::Trust` and `Knowledge::ContextBundle::Build`; `BuildForIssue` internal sections still use string concatenation | #3377 |
| Phase 4: Route PR and goal wrappers | Done — `BuildForPr` fully assembled; goal wrappers in `GoalAssembly` | #3378, #3379 |
| Phase 5: Expose safe customization | Done — `Profile`, `ProfileResolution`, budgets | #3380 |
| Phase 6: Closeout and cleanup | This audit | #3381 |

### Audit confirmations

- **Safety sections always included**: `Section#required?` → `safety_sensitive?`; `Profile#section_enabled?` always returns true for required sections; `Profile#ordered_sections` keeps required sections before optional. Verified by `build_spec.rb` and `goal_assembly_spec.rb`.
- **Trusted data only as instructions**: `Trust.comment_trusted?` / `classify_comment` fail-closed classification; excluded sections drop bodies; quarantined sections render with quarantine notice. Verified by `trust_spec.rb`, `build_spec.rb`, `build_for_pr_spec.rb`.
- **Provenance completeness**: `Result#provenance` includes digest, section list (key, source, trust_level, required, inclusion_reason), skipped sections, budget decisions, and prompt_digest. Persisted via `AgentRun#record_prompt_assembly!` and `PreparePrPromptActivity` phase metadata. Surfaced via `RunProvenanceBuilder`.
- **Customization limited to optional sections**: `Profile#normalize_budgets` restricts budgetable sections to `knowledge`, `style_guides`, `marketplace`. Required sections are immune to disabling, reordering, and budgeting.

### Documented follow-up

**BuildForIssue internal section migration (#3377 remainder):** `Prompts::BuildForIssue` uses `PromptAssembly::Trust` for comment filtering and produces quarantined knowledge context, but its own section composition still uses string concatenation. The trust boundary is enforced (comments are allowlist-filtered, knowledge is quarantined, base prompt is wrapped by `GoalAssembly` at runner time). Migrating `BuildForIssue`'s internal sections to explicit `PromptAssembly::Section`s — and converting `Lid::InjectIntoPrompt`, `StyleGuides::InjectIntoPrompt`, and `ProjectConventions::InjectIntoPrompt` into registered section providers — is filed as a follow-up to avoid changing prompt text in the audit pass.

**Configuration bundle fingerprint integration (Phase 6 validation criterion):** assembly provenance is persisted on the agent-run phase metadata rather than the configuration bundle, because the bundle is assigned and frozen at queue time (`CreateAgentRunActivity`) before prompt assembly runs at execution time (`RunAgentActivity`). The assembly digest and profile fingerprint are fully auditable on the run via `RunProvenanceBuilder`; folding them into a post-run bundle update is filed as a follow-up.

## References

- `app/services/prompt_assembly/` — `Build`, `Section`, `Result`, `Profile`, `ProfileResolution`, `Trust`, `TrustedInput`, `GoalAssembly`
- `app/models/agent_run.rb` — `effective_prompt`, `prompt_for_goal`, `record_prompt_assembly!`, custom prompt selection
- `app/services/prompts/build_for_issue.rb` — issue implementation prompt builder
- `app/services/prompts/build_for_pr.rb` — PR follow-up prompt builder (fully assembled via `PromptAssembly::Build`)
- `app/temporal/activities/create_agent_run_activity.rb` — queue-time prompt materialization
- `app/temporal/activities/run_agent_activity.rb` — runner-time goal augmentation (`PromptAssembly::GoalAssembly`)
- `app/temporal/activities/prepare_pr_prompt_activity.rb` — PR prompt assembly provenance persistence
- `app/services/run_provenance_builder.rb` — prompt assembly provenance display
- `app/services/style_guides/inject_into_prompt.rb` — style-guide section injection and exposure recording
- `app/services/knowledge/context_bundle/build.rb` — knowledge section ordering, budgets, and quarantine notice
...
- `docs/intent/prompt-assembly/` — LLD and EARS specs
