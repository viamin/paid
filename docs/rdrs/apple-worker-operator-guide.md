...
| Preflight reports a `gap` on `capacity`. | Free host disk is below the operator minimum, free memory is below 25%, or more than one Apple VM is active. | Free disk (`df -g /`); reconcile orphan VMs (`bin/paid apple-worker reconcile --destroy-orphans`); verify free memory (`vm_stat \| awk '/free/ {print $3}'`); stop other workloads on the host, then re-run `--preflight`. |
