...
class AddAppleVerificationAttemptReferences < ActiveRecord::Migration[8.1]
  ...
  def down
    remove_reference :execution_audit_events, :apple_verification_attempt, index: { algorithm: :concurrently } if column_exists?(:execution_audit_events, :apple_verification_attempt_id)
    remove_reference :execution_resource_ledger_entries, :apple_verification_attempt, index: { algorithm: :concurrently } if column_exists?(:execution_resource_ledger_entries, :apple_verification_attempt_id)
  end
end
