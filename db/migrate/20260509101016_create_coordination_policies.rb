...
class CreateCoordinationPolicies < ActiveRecord::Migration[8.1]
  def up
    safety_assured do
      unless table_exists?(:coordination_policies)
      ...
      unless table_exists?(:coordination_policy_versions)
      ...
      unless column_exists?(:coordination_policies, :current_version_id)
      ...
      add_index :coordination_policies, [ :account_id, :policy_type, :status ],
        name: "idx_coordination_policies_account_type_status" unless index_exists?(:coordination_policies, name: "idx_coordination_policies_account_type_status")
      add_index :coordination_policies, [ :project_id, :policy_type, :status ],
        name: "idx_coordination_policies_project_type_status" unless index_exists?(:coordination_policies, name: "idx_coordination_policies_project_type_status")
      add_index :coordination_policies, [ :account_id, :policy_type, :policy_key ],
        unique: true,
        where: "project_id IS NULL",
        name: "idx_coordination_policies_account_scope_key" unless index_exists?(:coordination_policies, name: "idx_coordination_policies_account_scope_key")
      add_index :coordination_policies, [ :account_id, :project_id, :policy_type, :policy_key ],
        unique: true,
        where: "project_id IS NOT NULL",
        name: "idx_coordination_policies_project_scope_key" unless index_exists?(:coordination_policies, name: "idx_coordination_policies_project_scope_key")

      add_index :coordination_policy_versions, [ :coordination_policy_id, :version ],
        unique: true,
        name: "idx_coordination_policy_versions_unique_version" unless index_exists?(:coordination_policy_versions, name: "idx_coordination_policy_versions_unique_version")
      add_index :coordination_policy_versions, :coordination_policy_id,
        unique: true,
        where: "status = 'active'",
        name: "idx_coordination_policy_versions_one_active" unless index_exists?(:coordination_policy_versions, name: "idx_coordination_policy_versions_one_active")
      add_index :coordination_policy_versions, [ :coordination_policy_id, :status, :created_at ],
        name: "idx_coordination_policy_versions_policy_status_created" unless index_exists?(:coordination_policy_versions, name: "idx_coordination_policy_versions_policy_status_created")
  ...
  def down
    safety_assured do
      ...
      if table_exists?(current_schema_table_name("coordination_policies"))
  ...
  def enable_row_level_security
    execute "DROP POLICY IF EXISTS tenant_isolation ON coordination_policy_versions" if table_exists?(:coordination_policy_versions)
    execute <<~SQL
      ALTER TABLE coordination_policies ENABLE ROW LEVEL SECURITY;
      ALTER TABLE coordination_policies FORCE ROW LEVEL SECURITY;
      DROP POLICY IF EXISTS tenant_isolation ON coordination_policies;
