...
ActiveRecord::Schema[8.1].define(version: 2026_08_19_013501) do
  ...
  create_table "egress_allowlist_entries", comment: "Tenant-managed host allowlist entries that resolve into an agent run's egress policy snapshot.", force: :cascade do |t|
    ...
    t.index ["account_id", "host_pattern", "scheme", "port"], name: "idx_egress_allowlist_entries_account_host_unique", unique: true, where: "(project_id IS NULL)"
    ...
    t.index ["project_id", "host_pattern", "scheme", "port"], name: "idx_egress_allowlist_entries_project_host_unique", unique: true, where: "(project_id IS NOT NULL)"
