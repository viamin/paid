...
# docker build -t paid .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name paid paid
...
ARG PGDG_POSTGRESQL_CLIENT_16_PACKAGE=postgresql-client-16=16.15-1.pgdg13+2
...
RUN BUNDLER_VERSION="$(awk '/^BUNDLED WITH$/{getline; gsub(/^ +/, "", $0); print $0}' Gemfile.lock)" && \
