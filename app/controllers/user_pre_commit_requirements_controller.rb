...
class UserPreCommitRequirementsController < ApplicationController
  ...
  def create
    @pre_commit_requirement = current_user.pre_commit_requirements.build(
      pre_commit_requirement_params
    )
    authorize @pre_commit_requirement

    if @pre_commit_requirement.save
      redirect_to user_pre_commit_requirements_path,
