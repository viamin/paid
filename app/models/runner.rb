...
class Runner < ApplicationRecord
  ...
  DIRECT_OUTBOUND_API_PROVIDERS = {
    ...
    # opencode_custom marks this as needing a full self-contained provider
    # config block because opencode's catalog lacks glm-5.x; the config is
    # generated by opencode_runner_runtime regardless of the built-in provider.
  ...
  def opencode_runner_runtime
    ...
    if api_config[:opencode_npm] == "@ai-sdk/anthropic"
      ...
      # Custom OpenAI-compatible endpoints flagged :opencode_custom (e.g. z.ai
      # coding plan) ship models opencode's catalog does not (glm-5.x), and some
      # opencode builds do not recognize the provider at all. Declare a
      # self-contained provider config — SDK, base URL, env-var API key, and the
      # configured model — so opencode resolves the model regardless of its
      # built-in catalog. Mirrors the @ai-sdk/anthropic path above and the
      # KiloCode provider config. The base URL lives in options, NOT in
      # OPENAI_BASE_URL, which would only redirect the built-in openai provider.
      provider_config["npm"] = "@ai-sdk/openai-compatible"
      options = opencode_openai_compatible_options(api_config, env_var)
      provider_config["options"] = options if options.any?
      models = opencode_custom_provider_models
      provider_config["models"] = models if models
    ...
    unless provider_config.empty?
      ...
      # :opencode_custom OpenAI-compatible entries; both always declare
      # opencode_model_provider. fetch (not ||) so a future entry that forgets
      # it fails loudly instead of silently mislabeling the block.
  ...
  # coding plan). Their model ids are not in opencode's catalog (glm-5.x) and
  # the provider may not be built into every opencode build, so a full
  # self-contained provider config is emitted instead of relying on the
  # built-in provider + OPENAI_BASE_URL.
  def custom_openai_compatible_provider?(api_config)
    api_config[:opencode_custom] == true
  end

  # opencode provider "options" for a custom OpenAI-compatible endpoint: the
  # base URL and an env-var-interpolated API key. opencode expands {env:NAME}.
  def opencode_openai_compatible_options(api_config, env_var)
    options = {}
    options["baseURL"] = api_config[:base_url] if api_config[:base_url].present?
    options["apiKey"] = "{env:#{env_var}}" if env_var.present?
    options
  end
