  ...
  class ScanPaidPrsActivity < BaseActivity
    ...
    def scan_draft_pr(project, client, issue, pr_data: nil)
      ...
      if all_triggers.empty? && !skip_comment_signals
        last_code_run = last_completed_code_run(project, issue)
        all_triggers.concat(check_conversation_comments(client, project, issue, last_code_run))

        # Only check changes_requested if still no triggers.
        if all_triggers.empty?
          reviews ||= fetch_reviews(client, project, issue)
          all_triggers.concat(changes_requested_from_reviews(project, reviews, last_code_run))
        end
      end
