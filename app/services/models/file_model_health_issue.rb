...
module Models
  ...
  # problems: catalog drift from DetectCatalogDrift and broken runner models from
  ...
  class FileModelHealthIssue
    LABEL = "model-health"
    LABEL_COLOR = "5319e7"
    LABEL_DESCRIPTION = "Provider model availability drift or broken runner models detected by Paid"
    ...
    def initialize(project:, drift:, broken:, client: nil)
      @project = project
      @drift = drift
      @broken = broken
    ...
    def summary_sections
      [ drift_section, broken_section ].compact.join("\n\n")
    end
    ...
    def combined_fingerprint
      Digest::SHA256.hexdigest("#{@drift.fingerprint}|#{@broken.fingerprint}")
    end
