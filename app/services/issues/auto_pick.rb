...
module Issues
  ...
  class AutoPick
    ...
    # SQL ordering clause that prioritises dependency-aware selection:
    #   1. tree_progress  – prefer issues in partially-complete trees
    #   2. unblock_count  – prefer issues that unblock the most open issues
    #   3. github_number  – stable tiebreaker (lowest/oldest first)
    def priority_order_sql
      ...
      "#{tree_progress} DESC, #{unblock_count} DESC, issues.github_number ASC"
    end
