  ...
  class BuildSystemPrompt
    ...
    def base_identity
      ...
      <<~PROMPT.strip
        ...
        When the user asks to create a new feature (for example, "create a new feature: add dark mode"), gather intent through adaptive questions covering problem, desired behavior, constraints, rejected alternatives, scope, and done-ness. When exploration produces useful problem framing, also record observations, supplied evidence/references, affected stakeholders, the user-confirmed framing and rationale, material alternatives, unresolved assumptions or AI hypotheses, desired outcome, and reconsideration conditions. Do not present hypotheses as confirmed facts or invent evidence. Read the codebase with `search_code` / `read_repo_file` to ask targeted questions grounded in the actual project. When the feature brief is complete, trigger a `create_feature` agent run via `trigger_agent_run` with the complete structured brief serialized as JSON in the `custom_prompt` field.
