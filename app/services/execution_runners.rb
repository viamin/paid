...
module ExecutionRunners
  ...
  # A single credential authority grant entry. Represents one credential class
  # a run is allowed to receive or access, carrying only non-secret metadata
  # (provider hints, scope identifiers, etc.) — never the secret value itself.
  #
  # +credential_class+ values:
  #   :paid_proxy_token       — Paid secrets proxy authentication token
  #   :github_authority       — GitHub push and PR authority (deploy key / token)
  #   :model_provider_proxy   — Model-provider credentials via Paid proxy (RDR-006)
  #   :model_provider_direct  — Model-provider direct outbound credentials
  #   :subscription_auth      — Host-backed subscription auth material
  #                             (Claude/Codex/Gemini/Copilot login)
  #   :mcp_credentials        — MCP server credentials
  #   :object_storage_upload  — Object-storage upload authority for artifacts
  #   :service_credentials    — Service container access credentials
  #
  # +metadata+ — Hash of non-secret metadata only. Callers must never place
  # token values, API keys, or other secret material in the metadata; the grant
  # is designed to be persisted, logged, and audited without redaction.
  # @spec CONTAINER-RUNTIME-018
  CredentialGrant = Data.define(:credential_class, :metadata) do
    def self.build(credential_class, metadata: {})
      new(credential_class: credential_class, metadata: metadata || {})
    end
  end

  # Constants assigned inside a Data.define block are lexically scoped to the
  # enclosing module, so we assign here after the class is created.
  CredentialGrant::CREDENTIAL_CLASSES = {
    paid_proxy_token: "Paid secrets proxy authentication token",
    github_authority: "GitHub push and PR authority",
    model_provider_proxy: "Model-provider credentials via Paid proxy (RDR-006)",
    model_provider_direct: "Model-provider direct outbound credentials",
    subscription_auth: "Host-backed subscription auth material",
    mcp_credentials: "MCP server credentials",
  ...
  CredentialGrant::CREDENTIAL_CLASSES.each_key do |key|
    CredentialGrant.define_method("#{key}?") { credential_class == key }
  end

  # The complete authority grant for a run — which credential classes the run
  # is allowed to receive or access, and how they flow (proxy, subscription,
  # or direct outbound). Carried on {RunSpec} so a runner can validate support
  # before provisioning and so grant decisions are explicit (RDR-056).
  #
  # The grant is secret-free by construction: {CredentialGrant} carries only
  # non-secret metadata. +#to_summary+ returns a hash suitable for logging and
  # audit without exposing any credential value.
  #
  # +mode+ values:
  #   :proxy             — RDR-006 default: provider calls through Paid's proxy
  #   :subscription_auth — Host-backed provider CLI auth (Claude/Codex/etc.)
  #   :direct_outbound   — Provider bypasses the proxy entirely
  #
  # Factory methods seed the baseline grants for each mode; additional grants
  # (MCP, object storage, service credentials) are passed by callers that
  # provision those resources.
  # @spec CONTAINER-RUNTIME-018
  AuthorityGrant = Data.define(:mode, :grants) do
    # RDR-006 default. Provider calls flow through Paid's secrets proxy.
    # The run receives a proxy token, GitHub authority, and proxy-mode
    # model-provider credentials.
    def self.proxy(grants: [])
      new(mode: AuthorityGrant::PROXY_MODE, grants: default_proxy_grants + Array(grants))
    end

    # Host-backed provider auth. The run receives subscription auth material
    # alongside the proxy token and GitHub authority.
    def self.subscription_auth(grants: [])
      new(mode: AuthorityGrant::SUBSCRIPTION_AUTH_MODE, grants: default_subscription_auth_grants + Array(grants))
    end

    # Provider bypasses the proxy entirely. The run receives direct-outbound
    # model-provider credentials and GitHub authority.
    def self.direct_outbound(grants: [])
      new(mode: AuthorityGrant::DIRECT_OUTBOUND_MODE, grants: default_direct_outbound_grants + Array(grants))
    end

    def proxy?
      mode == AuthorityGrant::PROXY_MODE
    end

    def subscription_auth?
      mode == AuthorityGrant::SUBSCRIPTION_AUTH_MODE
    end

    def direct_outbound?
      mode == AuthorityGrant::DIRECT_OUTBOUND_MODE
    end

    # Returns the set of credential classes granted by this authority.
    # @return [Array<Symbol>]
    def granted_classes
      grants.map(&:credential_class)
    end

    # Returns true when the given credential class is granted.
    def granted?(credential_class)
      grants.any? { |grant| grant.credential_class == credential_class }
    end

    # Returns the {CredentialGrant} for the given credential class, or nil.
    def grant_for(credential_class)
      grants.find { |grant| grant.credential_class == credential_class }
    end

    # Secret-free summary suitable for logging, audit, and persisted manifests.
    # Contains only the mode and credential-class identifiers — never token
    # values, keys, or other secret material.
    # @return [Hash]
    def to_summary
      { "mode" => mode.to_s, "credential_classes" => granted_classes.map(&:to_s) }
    end

    def self.default_proxy_grants
      [
        CredentialGrant.build(:paid_proxy_token),
        CredentialGrant.build(:github_authority),
        CredentialGrant.build(:model_provider_proxy)
      ]
    end

    def self.default_subscription_auth_grants
      [
        CredentialGrant.build(:paid_proxy_token),
        CredentialGrant.build(:github_authority),
        CredentialGrant.build(:subscription_auth)
      ]
    end

    def self.default_direct_outbound_grants
      [
        CredentialGrant.build(:github_authority),
        CredentialGrant.build(:model_provider_direct)
      ]
    end

    private_class_method :default_proxy_grants, :default_subscription_auth_grants, :default_direct_outbound_grants
  end

  # Mode constants are scoped to AuthorityGrant after the class is created for
  # the same lexical-scoping reason as CredentialGrant::CREDENTIAL_CLASSES.
  AuthorityGrant::PROXY_MODE = :proxy
  AuthorityGrant::SUBSCRIPTION_AUTH_MODE = :subscription_auth
  AuthorityGrant::DIRECT_OUTBOUND_MODE = :direct_outbound
  ...
  RunSpec = Data.define(
    ...
    :authority_grant,    # AuthorityGrant (credential classes the run may access)
    ...
    # @param authority_grant [AuthorityGrant, nil] credential authority grant
    # @param options [Hash] container options (memory_bytes, cpu_quota, etc.)
    # @return [RunSpec]
    def self.from_agent_run(agent_run, networking_policy: nil, authority_grant: nil, **options)
