...
module ConfigurationExperiments
  class Analyze
    ...
    attr_reader :configuration_experiment

    def initialize(configuration_experiment:)
    ...
    def all_have_minimum_samples?(variants)
      variants.all? { |variant| variant.sample_count >= configuration_experiment.min_samples_per_variant }
    end

    def scores_for(variant)
      variant.configuration_experiment_assignments
    ...
    def result_for(control_scores, variant)
      ...
      t_result = AbTests::Statistics.welch_t_test(control_scores, variant_scores)
      {
        ...
        significant: t_result[:p_value] < (1 - configuration_experiment.confidence_threshold)
      }
    end
