...
- **Avoid `safety_assured` in migrations** — The `strong_migrations` gem flags dangerous operations for good reason. Wrapping them in `safety_assured` silences the warning without fixing the underlying risk. When `strong_migrations` raises a warning, restructure the migration to be safe (e.g., use multi-step column additions, backfill in batches, add indexes concurrently) rather than suppressing the check. The only acceptable uses of `safety_assured` are for operations that are genuinely safe but not recognized by the gem (e.g., adding a check constraint alongside a validated column). If you must use it, leave a comment explaining why the operation is safe.
