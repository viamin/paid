...
**Status**: Phase 2 (Intelligence) complete. The system has prompt versioning, A/B testing, model meta-agent, cost tracking, quality metrics, semantic code search, and live dashboards. Phase 3 (Scale) is next.

## Git Workflow

- **The `main` branch is protected** - Never commit directly to `main`. Always create a feature branch and open a pull request. This means: (1) `git worktree add ../<branch-name> -b <branch>` before any changes (prefer worktrees over `git checkout -b` to avoid losing uncommitted work on `main`), (2) push the branch, (3) use `gh pr create` to open a PR. Never `git push` while on `main`. If you accidentally commit on `main`, move the commit to a feature branch before pushing. Remove worktrees when done with `git worktree remove`.
