...
- **The `main` branch is protected** - Never commit directly to `main`. Always create a feature branch and open a pull request. This means: (1) `git checkout -b <branch>` before any changes, (2) push the branch, (3) use `gh pr create` to open a PR. Never `git push` while on `main`. If you accidentally commit on `main`, move the commit to a feature branch before pushing.
